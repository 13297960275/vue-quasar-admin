  ...
  <div>
    <q-list v-show="!shrink" separator>
      <template v-for="menu in menuList"><!--  v-if="menu.leftMemu" -->
        <q-collapsible indent v-if="menu.children&&menu.children.length>0" :icon="menu.icon" :label="$t(menu.title)"
          :key="menu.name">
          <template v-for="child in menu.children"><!--  v-if="child.leftMemu" -->
            <q-collapsible v-if="child.children&&child.children.some(s=>{return s.leftMemu})" :key="child.name"
              :icon="child.icon" indent :label="$t(child.title)">
          ...
          <q-item-side :icon="menu.icon">
            <q-popover anchor="top right" self="top left">
              <q-list style="min-width: 100px">
                <template v-for="child in menu.children"><!--  v-if="child.leftMemu" -->
                  <q-item v-if="child.children&&child.children.some(s=>{return s.leftMemu}) >0"
                    :key="'shrink-'+child.name" v-ripple>
                    <q-item-side :icon="child.icon" />
